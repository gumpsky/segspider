...
deeplabv3_plus = DeepLabv3_plus(nInputChannels=3, n_classes=2, os=16, _print=True)
deeplabv3_plus = deeplabv3_plus.to(device)
criterion = nn.NLLLoss().to(device)
optimizer = optim.Adam(deeplabv3_plus.parameters(), lr=1e-4)
...
def train(model):
    ...
    for epoch in range(cfg.EPOCH_NUMBER):
        ...
        if max(best) <= train_miou / len(train_data):
            ...
            t.save(net.state_dict(), './deeplabv3_plus_train/model{}.pth'.format(epoch))
